...
long long greatestProduct(const std::vector<std::vector<int>>& grid)
{
    Timer tr;
    ...
    for (int i = 0; i < gridSize; ++i)
    {
        for (int j = 0; j < gridSize; ++j)
        {
            ...
            if (j < gridSize - 3)
            {
                long long product = static_cast<long long>(grid[i][j]) * grid[i][j + 1] * grid[i][j + 2] * grid[i][j + 3];
                maxProduct = std::max(maxProduct, product);
            }
            // Vertical down
            if (i < gridSize - 3)
            {
                long long product = static_cast<long long>(grid[i][j]) * grid[i + 1][j] * grid[i + 2][j] * grid[i + 3][j];
                maxProduct = std::max(maxProduct, product);
            }
            // Diagonal down-right
            if (i < gridSize - 3 && j < gridSize - 3)
            {
                long long product = static_cast<long long>(grid[i][j]) * grid[i + 1][j + 1] * grid[i + 2][j + 2] * grid[i + 3][j + 3];
                maxProduct = std::max(maxProduct, product);
            }
            // Diagonal down-left
            if (i < gridSize - 3 && j >= 3)
...
int main()
{
    ...
    std::cout << "The greatest product is: " << greatestProduct(grid) << std::endl;
}
...
01   08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08
