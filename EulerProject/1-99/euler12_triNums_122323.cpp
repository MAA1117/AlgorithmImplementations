...
{
    if (n % 2 && n % 3) return -1; // if it's not divisible by 2 or 3 dont bother
...
int divisorCount(long long n, std::vector<int>& divisorMemo)
{
    ...
    for (long long i = 1; i <= sqrt(n); i++)
    {
        if (n % i == 0)
        {
            divCount += (n / i == i) ? 1 : 2; // Add 2 for each pair of divisors, 1 if i is the square root
        }
    }
...
int main()
{
    ...
    for (int i = 3; i < 1'000'000; i++)
    {
        ...
        if (divisorCount(triNum) > target)
        {
            ...
            std::cout << "First triangle n with > " << target << " divisors: " << triNum << '\n';
            break;            
        }
    }
    std::cout << "(" << divCount << " divisors, triNum # " << n << ")" << std::endl;
}
